...
public class EditorEditableContainerSystem : FSystem 
{
	...
	private IEnumerator tcheckLinkName()
	{
		...
		foreach (var rob in f_paintables.First().GetComponent<PaintableGrid>().floorObjects.Values.Where(fo => fo.type == Cell.Player || fo.type == Cell.Enemy).Cast<Robot>())
		{
			validNames.Add(rob.associatedScriptName);
		}
